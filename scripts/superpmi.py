        ...
        with TempDir() as temp_location:
            ...
            if os.path.isfile(self.diff_mcl_file) and os.stat(self.diff_mcl_file).st_size != 0 or self.coreclr_args.diff_with_code_only:
                ...
                if len(text_differences) > 0:
                    print("Textual differences found, the asm is located under <coreclr_dir>/bin/asm/base and <coreclr_dir>/bin/asm/diff")
                    print("")
...
def setup_args(args):
    ...
    """
    coreclr_args = CoreclrArguments(args, require_built_core_root=True, require_built_product_dir=False, require_built_test_dir=False, default_built_type="Checked")
