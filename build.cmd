...
if /i "%1" == "-dockerbuild"         (set __DockerBuild=1&set processedArgs=!processedArgs! %1&shift&goto Arg_Loop)
...
:GenVSSolution
    ...
    if defined __DockerBuild set __ExtraCmakeArgs=!___SDKVersion! "-DCLR_CMAKE_TARGET_OS=%__BuildOs%" "-DCLR_CMAKE_PACKAGES_DIR=%__PackagesDir%" "-DCLR_CMAKE_PGO_INSTRUMENT=%__PgoInstrument%" "-DCLR_CMAKE_OPTDATA_VERSION=%__PgoOptDataVersion%" "-DCLR_CMAKE_PGO_OPTIMIZE=%__PgoOptimize%" "-DCLR_CMAKE_DOCKERBUILD=1"
...
echo -dockerbuild: HACK to enable building in a docker windows container
